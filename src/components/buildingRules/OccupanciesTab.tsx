...
import { OccupancyComparisonTable } from "./OccupancyComparisonTable";
import { OccupancyDetailDrawer } from "./OccupancyDetailDrawer";
import { StatutoryHighlightsSection } from "./StatutoryHighlightsSection";
import { HeightSetbackCalculator } from "./HeightSetbackCalculator";
import {
  Layers,
  Table,
  Grid,
...
export const OccupanciesTab: React.FC<OccupanciesTabProps> = ({ onAskAIWithContext }) => {
  const [activeSubView, setActiveSubView] = useState<"table" | "highlights" | "calculator" | "cards">("table");
  const [selectedOccupancy, setSelectedOccupancy] = useState<OccupancyComparisonRow | null>(null);
  ...
  const handleAskOccupancyAI = async (group: OccupancyGroup) => {
    const query =
      aiQueries[group.id] ||
      `കേരള കെട്ടിട നിർമ്മാണ ചട്ട പ്രകാരം ${group.code} (${group.nameMl}) കെട്ടിടത്തിന്റെ പ്രധാന അനുമതി പ്രക്രിയകളും നിയമങ്ങളും ചുരുക്കത്തിൽ വിശദീകരിക്കുക.`;
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
          <div className="space-y-1">
            ...
            <p className="text-xs text-slate-300 font-medium max-w-3xl leading-relaxed">
              Under the <strong>Kerala Panchayat Building Rules (KPBR), 2019</strong> (amended up to SRO No. 1241/2025),
              buildings are classified into <strong>Occupancy Groups A1 to J</strong> based on purpose, human occupancy density,
              and structural/environmental hazard levels.
            </p>
          </div>
          ...
          <div className="flex items-center gap-1.5 bg-slate-950 p-1.5 rounded-xl border border-slate-800 self-start lg:self-center">
            ...
            <button
              onClick={() => setActiveSubView("highlights")}
              className={`px-3 py-2 rounded-lg text-xs font-mono font-bold flex items-center gap-2 transition-all cursor-pointer ${
                activeSubView === "highlights"
                  ? "bg-emerald-500 text-slate-950 shadow-md"
                  : "text-slate-400 hover:text-white"
              }`}
            >
              <ShieldAlert className="w-4 h-4" />
              <span>Statutory Highlights</span>
            </button>
            <button
      ...
      {/* VIEW 1: COMPARATIVE TABLE (A1 TO J) */}
      {activeSubView === "table" && (
        <div className="space-y-6">
          <OccupancyComparisonTable
            onSelectOccupancy={(row) => setSelectedOccupancy(row)}
            onAskAI={(prompt) => {
              if (onAskAIWithContext) {
                onAskAIWithContext(prompt);
              } else {
                setActiveSubView("cards");
              }
            }}
          />

          {/* Inline Statutory Highlights below comparison table for comprehensive overview */}
          <div className="pt-4">
            <StatutoryHighlightsSection />
          </div>
        </div>
      )}

      {/* VIEW 2: STATUTORY HIGHLIGHTS & EXCEPTIONS */}
      {activeSubView === "highlights" && (
        <div className="space-y-6">
          <StatutoryHighlightsSection />
      ...
      {/* VIEW 3: HEIGHT SETBACK INCREMENT CALCULATOR */}
      {activeSubView === "calculator" && (
        <div className="space-y-6">
          <HeightSetbackCalculator />
          <StatutoryHighlightsSection />
      ...
      {/* VIEW 4: DETAILED OCCUPANCY CARDS & AI QUERY INSPECTOR */}
      {activeSubView === "cards" && (
        <div className="space-y-6">
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 bg-slate-900/60 p-4 rounded-xl border border-slate-800">
            <div>
              <h3 className="text-base font-bold text-white font-sans">
                Occupancy Profile Dossiers &amp; AI Queries
              </h3>
              <p className="text-xs text-slate-400">
                Explore individual occupancy profiles or query VasthuSilpy AI for building rules, setback relaxations, and fire NOCs.
              </p>
            </div>
            <div className="w-full md:w-80 relative">
              ...
              <input
                ...
                placeholder="Search Group (A1, B, Hospital, Shop...)"
      ...
      {/* MODAL / DRAWER FOR SELECTED OCCUPANCY */}
      {selectedOccupancy && (
        <OccupancyDetailDrawer
